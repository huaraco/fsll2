...
if( tie_get_option( 'slider' ) ):
	...
	if( $slider_query == 'custom' ){
		...
		elseif( $slider_query  == 'post'){
			...
			$args= array('posts_per_page'=> $number , 'post_type' => 'post', 'post__in' => $posts_var  );
		}
		elseif( $slider_query  == 'page'){
			$pages_var = explode (',' , tie_get_option('slider_pages'));
			$args= array('posts_per_page'=> $number , 'post_type' => 'page', 'post__in' => $pages_var  );
		}
	...
	<div id="ei-slider" class="ei-slider">
		...
		<?php $i= 0;
			...
			<?php if ( function_exists("has_post_thumbnail") && has_post_thumbnail() ) : ?>
				<a href="<?php the_permalink(); ?>"><img src="<?php echo tie_thumb_src( 'slider' ); ?>"  alt="<?php the_title(); ?>" /></a>
			<?php endif; ?>
				<div class="ei-title">
		...
		<?php $i= 0;
			while ( $featured_query->have_posts() ) : $featured_query->the_post(); $i++; ?>
			<li><a href="#">Slide <?php echo $i; ?></a><img src="<?php echo tie_thumb_src( 'tie-medium' ); ?>" alt="<?php the_title(); ?>" /></li>
    		<?php endwhile;?>
		</ul><!-- ei-slider-thumbs -->
	</div>
	<?php endif; ?>
