...
function get_latest_theme_version($interval) {
	...
	$db_cache_field_last_updated = 'notifier-cache-last-updated_sahifa';
	...
	if ( !$last || (( $now - $last ) > $interval) ) {
		...
		$cache = wp_remote_retrieve_body( wp_remote_get( $notifier_file_url , array( 'timeout' => 18 , 'sslverify' => false ) ) );
		...
		/*if( function_exists('curl_init') ) { // if cURL is available, use it...
			...
			file_get_contents($notifier_file_url); // ...if not, use the common file_get_contents()
		}*/
